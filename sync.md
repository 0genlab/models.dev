...
- The endpoint supplies effort domains, checked against model-specific lab documentation and peers. The known seven-value protocol fallback is conservatively omitted until the payload can distinguish verified model domains from protocol defaults. This is an evidence guard, not proof that seven native levels are impossible. Surviving toggle/budget controls remain; an effort-only unknown result retains authored controls or skips a new reasoner, never inventing `[]`. Incorrect source claims must still be corrected at the source.
